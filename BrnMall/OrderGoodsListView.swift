...
class OrderGoodsListView: UIView {
	...
	var order: OrderDetail? {
		didSet {
			goodsListView.order_goods = order!.proList
			...
			payMoneyLabel.text = "￥" + (order?.surplusMoney)!
			payLabel.text = "实付"
		}
	}
	...
	override init(frame: CGRect) {
		...
		payLabel.textColor = BMTextBlackColor
...
class OrderGoodsView: UIView {
	...
	var order_goods: [OrderDetailPro]? {
		didSet {
			for subview in self.subviews {
				subview.removeFromSuperview()
			}
			orderGoodsViewHeight = 0
			lastViewY = 0
			for i in 0 ..< order_goods!.count {
				let array = order_goods
				if array!.count >= 1 {
...
class GoodsView: UIView {
	...
	var orderGoods: OrderDetailPro? {
		didSet {
			titleLabel.text = orderGoods?.pname
			numberLabel.text = "x" + (orderGoods?.realCount)!
...
//			}
	...
	override init(frame: CGRect) {
		...
		titleLabel.textColor = BMTextBlackColor
		addSubview(titleLabel)
		
		numberLabel.font = UIFont.systemFontOfSize(14)
		numberLabel.textColor = BMTextBlackColor
		addSubview(numberLabel)
		
		priceLabel.font = UIFont.systemFontOfSize(14)
		priceLabel.textColor = BMTextBlackColor
	...
	override func layoutSubviews() {
		...
		if isShowImageView {
			...
			titleLabel.frame = CGRectMake(CGRectGetMaxX(giftImageView.frame) + 5, 0, width * 0.85, height)
			numberLabel.frame = CGRectMake(CGRectGetMaxX(titleLabel.frame) + 10 - 45, 0, 30, height)
		} else {
			titleLabel.frame = CGRectMake(10, 0, width * 0.85, height)
...
class FeeView: UIView {
	...
	override init(frame: CGRect) {
		...
		prictLabel.textColor = BMTextBlackColor
