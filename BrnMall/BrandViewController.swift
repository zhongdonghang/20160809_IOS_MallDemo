...
class BrandViewController: BaseViewController, TitleViewProtocol {

	private var categoryLay1_Data: [CategoryLay1] = []
	private var categoryTableView: BMTableView!
	private var brandVC: BrandListViewController!
	...
	private func hideEmptyUI() {
		...
		self.brandVC.view.hidden = false
	}
	// MARK:- Creat UI
	...
	private func bulidBrandListViewController() {
		brandVC = BrandListViewController()
		// productsVC.delegate = self
		brandVC.view.hidden = true
		addChildViewController(brandVC)
		view.addSubview(brandVC.view)
		...
		manager.request(APIRouter.getCategoryLay1).responseJSON(completionHandler: {
			...
			case .Success:
				...
				if self.brandVC != nil {
					...
					self.brandVC.curID = self.categoryLay1_Data[0].cateId
					self.brandVC.clickCategotyItem()
				}
			}
		})
	}
...
extension BrandViewController: UITableViewDelegate, UITableViewDataSource {
	...
	func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
		if brandVC != nil {
			...
			brandVC.curID = categoryLay1_Data[indexPath.row].cateId
			brandVC.clickCategotyItem()
		}
	}
}