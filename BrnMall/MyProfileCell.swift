...
class MyProfileCell: UITableViewCell {
	...
	static private let identifier = "MyProfileCell"

	class func cellFor(tableView: UITableView) -> MyProfileCell {

		var cell = tableView.dequeueReusableCellWithIdentifier(identifier) as? MyProfileCell

		if cell == nil {
			cell = MyProfileCell(style: .Default, reuseIdentifier: identifier)
		}
	...
	override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
		...
		detaildeLabel.font = UIFont.systemFontOfSize(16)
	...
	override func layoutSubviews() {
		...
		detaildeLabel.frame = CGRectMake(110, 0, ScreenWidth - 135 - (arrowView.image?.size.width)!, height)
		let leftMarge: CGFloat = 20
		bottomLine.frame = CGRectMake(leftMarge, height - 0.5, width - leftMarge, 0.5)
	}
}

class MyProfileCellModel: NSObject {

	var title: String?
	var detail: String?

	init(Title title: String, Detail detail: String) {
		self.title = title
		self.detail = detail
	}
}
